<%@ Page Title="" Language="C#" MasterPageFile="~/n_ter_base_loggedin_grid.master" AutoEventWireup="true" CodeBehind="calenders.aspx.cs" Inherits="N_Ter_Tasks.calenders" %>
...
<asp:Content ID="Content4" ContentPlaceHolderID="contBody" runat="Server">
    <asp:HiddenField ID="hndCalender_ID" runat="server" />
    <div class="panel panel-info">
        <div class="panel-body">
            <div class="padding-xs-vr">
                <button id="cmdNew" runat="server" class="btn btn-labeled btn-primary"><span class="btn-label icon fa fa-plus"></span>New Calendar</button>
            ...
            <div class="table-responsive table-primary no-margin-b">
                <asp:GridView ID="gvMain" runat="server" CssClass="table table-striped table-hover grid_table grid_calender non_full_width_table" data-size="non_full_width_table" AutoGenerateColumns="False" OnRowDataBound="gvMain_RowDataBound">
                    <Columns>
                        <asp:BoundField DataField="Calender_ID" />
                        <asp:TemplateField>
                            <ItemTemplate>
                        ...
                        <asp:BoundField DataField="Calender_Name" HeaderText="Calendar Name" />
                    </Columns>
                </asp:GridView>
            </div>
        </div>
        ...
        <div id="at_model_indent" class="at_modelpopup_indent">
            <div id="at_model_inner_indent" class="panel panel-inverse at_modelpopup_inner_indent">
                <div class="panel-heading">
                    ...
                    <h4 class="panel-title">Add/Edit Calendar</h4>
                </div>
                <div id="at_model_content" class="at_modelpopup_content styled-bar">
                    <div class="panel-body">
                        <div class="form-group">
                            <label>Calendar Name</label>
                            <asp:TextBox ID="txtCalenderName" runat="server" CssClass="form-control"></asp:TextBox>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label>User Group - View</label>
                            <asp:DropDownList ID="cboUserGroup" runat="server" CssClass="form-control"></asp:DropDownList>
                        </div>
                        <div class="form-group">
                            <asp:CheckBox runat="server" ID="chkAllowConflicts" CssClass="checkboxlist" /><span style="padding-left: 10px">Allow Conflicts</span>
                        </div>
                    </div>
                </div>
...
<asp:Content ID="Content5" ContentPlaceHolderID="contScripts" runat="Server">
    <script type="text/javascript">
        ...
        function ClearControls() {
            clearTextBox(['<%=txtCalenderName.ClientID%>', '<%=txtSubject.ClientID%>', '<%=txtDescription.ClientID%>']);
            clearDropDown(['<%=cboUserGroup.ClientID%>']);
            clearCheckBox(['<%=chkAllowConflicts.ClientID%>']);
            return false;
        }

        function ResetControls() {
            if ($("#<%=hndCalender_ID.ClientID%>").val().trim() == '0') {
                ClearControls();
            }
            else {
                LoadValues();
        ...
        function LoadValues() {
            $.ajax({
                type: "GET",
                url: "api/tasks/GetCalender",
                data: { Calender_ID: $("#<%=hndCalender_ID.ClientID%>")[0].value },
        ...
        function LoadValauesPassed(result) {
            ...
            $('#<%=txtCalenderName.ClientID%>').val(resultObject[0].Calender_Name);
            $('#<%=txtSubject.ClientID%>').val(resultObject[0].Default_Subject);
            $('#<%=txtDescription.ClientID%>').val(resultObject[0].Default_Description);
            $('#<%=cboUserGroup.ClientID%>').val(resultObject[0].View_User_Group_ID);
            if (resultObject[0].Allow_Conflicts == '1') {
                onCheckBox('#<%=chkAllowConflicts.ClientID%>');
            }
        }
    </script>
</asp:Content>