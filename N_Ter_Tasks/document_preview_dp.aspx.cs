...
{
    public partial class document_preview_dp : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (Session["dt"] != null)
                {
                    ...
                    if (objSes != null && (objSes.UserID > 0 || objSes.UserID == -1))
                    {
                        N_Ter.Security.URL_Manager objURL = new N_Ter.Security.URL_Manager();
                        int DocID = Convert.ToInt32(objURL.Decrypt(Request.QueryString["fid"]));

                        Documents objDocument = ObjectCreator.GetDocuments(objSes.Connection, objSes.DB_Type, objSes.PhysicalRoot, objSes.WebRoot, objSes.EnableReading);
                        DS_Documents.tbldocumentsRow dr = objDocument.ReadDocument(DocID);
                        string FilePath = dr.Document_Path;
