...
{
    public partial class n_ter_base_loggedin_grid_task_list_t2 : System.Web.UI.MasterPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (IsPostBack == false)
            {
                ...
                PopulateDropdowns(Queue_ID, WF_Category, Walkflow_ID, objSes);
            }

            ReturnPage = Request.FilePath.Substring(1);
        }

        private void PopulateDropdowns(int Queue_ID, int Req_WFCategory_ID, int Req_Workflow_ID, SessionObject objSes)
        {
            N_Ter.Security.URL_Manager objURL = new N_Ter.Security.URL_Manager();
            ...
            DS_Tasks dsTasks = objTasks.ReadAll(objSes.UserID, isActive, objCus);
            ...
            if (dsAltUsers.tblusers.Count > 0)
            {
                ...
                foreach (DS_Users.tblusersRow rowUser in dsAltUsers.tblusers)
                {
                    ...
                    dsTasksTemp = objTasks.ReadAll(rowUser.User_ID, isActive, objCus);
                            ...
                            foreach (DS_Workflow.tblworkflow_categoriesRow row in dsCats.tblworkflow_categories)
                            {
                                ...
                                if (drTasksInCat.Count > 0)
                                {
                                    ...
                                    if (row.Workflow_Category_ID == Req_WFCategory_ID)
                                    {
                                        ...
                                        objArgs.SelectedTasks = drTasksInCat;
