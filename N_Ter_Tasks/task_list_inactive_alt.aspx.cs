...
{
    public partial class task_list_inactive_alt : System.Web.UI.Page
    {
        ...
        private void ObjT3_FillTasks(object sender, EventArgs e)
        {
            ...
            List<DS_Tasks.tbltasksRow> drTasksInStep;
            ...
            DS_Workflow dsWFSteps = objWF.ReadAllStep(objArgs.Active_WF);

            foreach (DS_Workflow.tblworkflow_stepsRow rowStep in dsWFSteps.tblworkflow_steps)
            {
                drTasksInStep = objArgs.SelectedTasks.Where(x => x.Current_Step_ID == rowStep.Workflow_Step_ID).ToList();
                if (drTasksInStep.Count > 0)
                {
                    if ((objArgs.Active_WF_Step == 0 && TabIndex == 1) || (objArgs.Active_WF_Step == rowStep.Workflow_Step_ID))
                    {
                        TabCSS = " class='active'";
                    }
                    else
                    ...
                    ltrTabs.Text = ltrTabs.Text + "<li" + TabCSS + ">" + "\r\n" +
                                       "<a data-toggle='tab' href='#lft_tab_" + TabIndex + "'>" + rowStep.Step_Status + " <span id='stp_cnt_" + rowStep.Workflow_Step_ID + "' class='label label-info'>" + Convert.ToString(drTasksInStep.Count) + "</span></a>" + "\r\n" +
                                   "</li>" + "\r\n";
                    DrawTabs(drTasksInStep, TabIndex, objArgs.Queue_ID, rowStep.Workflow_Step_ID, objArgs.Active_WF_Step, objArgs.SelectedWF.Exrta_Field_Naming, objArgs.SelectedWF.Exrta_Field2_Naming, objSes);
                    TabIndex = TabIndex + 1;
                }
            }
            ...
            objT3.TabCountScript = (TabIndex - 1).ToString();
        }

        private void DrawTabs(List<DS_Tasks.tbltasksRow> drTasks, int TabIndex, int Queue_ID, int Workflow_Step_ID, int Req_Step_ID, string Extra1Name, string Extra2Name, SessionObject objSes)
        {
            if (drTasks.Count > 0)
            {
                ...
                if ((Req_Step_ID == 0 && TabIndex == 1) || (Req_Step_ID == Workflow_Step_ID))
                {
